...
func TestValidateSubGroups(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Lowercase with numbers and hyphens",
			subGroups: []SubGroup{
				{Name: "worker-pool-1", MinMember: 1},
				{Name: "gpu-nodes", Parent: ptr.To("worker-pool-1"), MinMember: 1},
			},
			wantErr: nil,
		},
		{
			name: "Uppercase caught before duplicate check",
			subGroups: []SubGroup{
				{Name: "A", MinMember: 1},
				{Name: "A", MinMember: 1}, // duplicate, but uppercase is caught first
			},
			wantErr: errors.New("subgroup name \"A\" must be lowercase"),
		},
	}
